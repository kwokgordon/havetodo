...
#  scope :today_tasks, -> { with_duedate_tasks.where(due_date: (Time.now.midnight)..(Time.now.midnight + 1.day)) }
  scope :tomorrow_tasks, -> { with_duedate_tasks.where(due_date: (Time.now.midnight + 1.day)..(Time.now.midnight + 2.day)) }
  scope :this_week_tasks, -> { with_duedate_tasks.where(due_date: (Time.now.midnight + 2.day)..(Time.now.midnight + 7.day)) }
